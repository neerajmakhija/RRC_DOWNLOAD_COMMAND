...

{
    ...
    buffer = (char*)malloc(application_size);
    if (buffer == NULL) {
        printf("Memory error.\n");
        return 1;
    }

    result = fread(buffer, 1, application_size, in);
    if (result != application_size) {
    ...
    //     result = fwrite(buffer, 1, application_size, out);
    //     if (result != application_size)
    //     {
    ...
    for (int i = 0; i < num_of_packets; i++)
    {
        ...
        //strncat(&rrc_download_application_idg_command[V2_APK_DATA_IDX], &buffer[num_of_bytes_written],num_of_bytes_to_write_in_buffer);
